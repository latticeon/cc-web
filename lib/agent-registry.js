...
const AGENT_DEFINITIONS = [
  ...
  // @deprecated CodeBuddy 已弃用，不再在 UI 中展示。底层运行时代码保留以向后兼容已有会话。
  // 如需恢复使用，移除 deprecated 标记即可。
  {
    id: 'codebuddy',
    label: 'CodeBuddy',
    avatar: '',
  ...
  // @deprecated Kimi 已弃用，不再在 UI 中展示。底层运行时代码保留以向后兼容已有会话。
  // 如需恢复使用，移除 deprecated 标记即可。
  {
    id: 'kimi',
    label: 'Kimi',
  ...
  // @deprecated OpenCode 已弃用，不再在 UI 中展示。底层运行时代码保留以向后兼容已有会话。
  // 如需恢复使用，移除 deprecated 标记即可。
  {
    id: 'opencode',
    label: 'OpenCode',
...
function getAgentIds() {
  // 只返回未弃用的 agent id
  return AGENT_DEFINITIONS.filter((agent) => !agent.deprecated).map((agent) => agent.id);
}
...
function getPublicAgentCatalog() {
  // 过滤掉已弃用的 agent（deprecated: true），不在 UI 中展示
