...
function createAgentRuntime(deps) {
  ...
  function buildWindowsCommandLine(cliPath, cliArgs) {
    return [quoteWindowsCmdArg(cliPath), ...cliArgs.map(quoteWindowsCmdArg)].join(' ');
  }
  ...
  function resolveWindowsCliCommand(cliPath, cliArgs) {
    ...
    if (/\.(cmd|bat)$/i.test(cliPath)) {
      return {
        command: 'cmd.exe',
        args: ['/d', '/s', '/c', buildWindowsCommandLine(cliPath, cliArgs)],
      };
    }
    // Windows cannot spawn a PATH-resolved npm command without its .cmd wrapper.
    // Resolve bare CLI names through cmd.exe, matching how they run in a shell.
    const isBareCommand = !/[\\/]/.test(cliPath) && !/\.[a-z0-9]+$/i.test(cliPath);
    if (isBareCommand) {
      return {
        command: 'cmd.exe',
        args: ['/d', '/s', '/c', buildWindowsCommandLine(cliPath, cliArgs)],
  ...
  function buildClaudeSpawnSpec(session, options = {}) {
    ...
    const aiSelection = resolveAiSessionSelection(session);
    if (aiSelection) {
      const modelValue = aiSelection.provider.kind === 'local'
        ? aiSelection.model.id
        : `${aiSelection.provider.id}/${aiSelection.model.id}`;
    ...
    if (aiSelection?.provider.kind === 'remote') {
      env.ANTHROPIC_BASE_URL = `http://127.0.0.1:${processEnv.CC_WEB_PORT || processEnv.PORT || 8002}/router/anthropic`;
      env.ANTHROPIC_API_KEY = getAiVirtualApiKey();
      env.ANTHROPIC_AUTH_TOKEN = getAiVirtualApiKey();
    }
  ...
  function buildCodexSpawnSpec(session, options = {}) {
    const runtimeConfig = prepareCodexAiRuntime(session);
    ...
    const aiSelection = runtimeConfig.selection || resolveAiSessionSelection(session);
    const effectiveModel = aiSelection
      ? (aiSelection.provider.kind === 'local' ? aiSelection.model.id : `${aiSelection.provider.id}/${aiSelection.model.id}`)
      : session.model;
    if (effectiveModel) {
      ...
      args.push('--model', raw);
      const level = String(session.reasoningEffort || '').trim().toLowerCase();
      if (['low', 'medium', 'high', 'xhigh'].includes(level)) args.push('-c', `model_reasoning_effort=${level}`);
    }
  ...
  function streamSend(entry, sessionId, data) {
    if (!entry?.turnMetrics) {
      entry.turnMetrics = { firstTextAt: null, toolStartedAt: new Map(), toolDurationMs: 0 };
    }
    if (data?.type === 'text_delta' && !entry.turnMetrics.firstTextAt) {
      entry.turnMetrics.firstTextAt = Date.now();
    }
    if (data?.type === 'tool_start' && data.toolUseId) {
      entry.turnMetrics.toolStartedAt.set(data.toolUseId, Date.now());
    }
    if (data?.type === 'tool_end' && data.toolUseId) {
      const startedAt = entry.turnMetrics.toolStartedAt.get(data.toolUseId);
      if (startedAt) {
        entry.turnMetrics.toolDurationMs += Math.max(0, Date.now() - startedAt);
        entry.turnMetrics.toolStartedAt.delete(data.toolUseId);
      }
    }
    if (!entry?.ws) return;
    wsSend(entry.ws, { ...data, sessionId });
  }

  function completePendingToolCalls(entry) {
    for (const toolCall of entry.toolCalls || []) {
      if (toolCall.done) continue;
      toolCall.done = true;
      streamSend(entry, entry.sessionId, {
        type: 'tool_end',
  ...
  function appendAssistantTextStep(entry, text) {
    ...
    streamSend(entry, entry.sessionId, { type: 'text_delta', text });
  }
  ...
  function processClaudeEvent(entry, event, sessionId) {
    ...
    if (entry && !entry.sessionId && sessionId) entry.sessionId = sessionId;
      ...
      case 'assistant': {
        ...
        for (const block of content) {
          if (block.type === 'text' && block.text) {
            ...
            entry.toolCalls.push(tc);
            attachAssistantToolStep(entry, tc);
            streamSend(entry, entry.sessionId, { type: 'tool_start', name: block.name, toolUseId: block.id, input: tc.input });
            ...
            streamSend(entry, entry.sessionId, { type: 'tool_end', toolUseId: block.tool_use_id, result: resultText.slice(0, 2000) });
          }
        }
      ...
      case 'result': {
        ...
        if (session) {
          ...
          if (usage && typeof usage === 'object') {
            ...
            const cachedInputTokens = Math.max(0, Number(usage.cache_read_input_tokens ?? usage.cached_input_tokens) || 0);
            const cacheCreationInputTokens = Math.max(0, Number(usage.cache_creation_input_tokens) || 0);
            const outputTokens = Math.max(0, Number(usage.output_tokens) || 0);
            session.totalUsage = {
              ...(session.totalUsage || {}),
              inputTokens: (session.totalUsage?.inputTokens || 0) + inputTokens,
              cachedInputTokens: (session.totalUsage?.cachedInputTokens || 0) + cachedInputTokens,
              cacheCreationInputTokens: (session.totalUsage?.cacheCreationInputTokens || 0) + cacheCreationInputTokens,
              outputTokens: (session.totalUsage?.outputTokens || 0) + outputTokens,
            ...
            streamSend(entry, sessionId, { type: 'usage', totalUsage: session.totalUsage });
          }
          saveSession(session);
        }
        entry.lastCost = event.total_cost_usd || null;
        if (entry.ws && event.total_cost_usd !== undefined) {
          streamSend(entry, sessionId, { type: 'cost', costUsd: session?.totalCost || 0 });
        }
        entry.protocolComplete = true;
        break;
      }
    }
  }
    ...
    switch (event.type) {
      ...
      case 'item.completed': {
        ...
        if (item.type === 'agent_message') {
          if (item.text) {
            completePendingToolCalls(entry);
      ...
      case 'turn.completed': {
        ...
        const session = loadSession(sessionId);
        if (session && usage) {
          const contextTokens = Number(resolveCodexContextTokens?.(session) || 0);
          session.totalUsage = {
            ...
            contextTokens: Number.isFinite(contextTokens) && contextTokens > 0 ? Math.round(contextTokens) : 0,
          };
          saveSession(session);
          streamSend(entry, sessionId, { type: 'usage', totalUsage: session.totalUsage });
        }
      ...
      case 'error':
        if (event.message) {
          ...
          if (/^Reconnecting\.\.\./.test(event.message)) {
            streamSend(entry, sessionId, { type: 'system_message', message: event.message });
          } else {
  ...
  function processKimiEvent(entry, event, sessionId) {
    ...
    if (role === 'assistant') {
      ...
      } else if (String(event.content || '').trim()) {
        entry.protocolComplete = true;
      }
      return;
    }
  ...
  function maybeUpdateCodebuddyModel(entry, sessionId, modelName) {
    ...
    saveSession(session);
    streamSend(entry, sessionId, { type: 'model_changed', model: nextModel });
  }
  ...
  function processOpencodeEvent(entry, event, sessionId) {
    ...
    if (partType === 'step-finish') {
      ...
      const finishReason = normalizeOpencodeName(part?.reason || event?.reason);
