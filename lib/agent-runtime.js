...
function createAgentRuntime(deps) {
  ...
  // 注意：kimi、codebuddy、opencode 的 runtime handlers 已弃用（deprecated），
  // 不再在新会话中使用，但保留以向后兼容已有会话的加载和续接。
  ...
  // 以下导出中，codebuddy/kimi/opencode 相关的函数已弃用（deprecated），
  // 保留导出以向后兼容已有会话，不再用于新建会话。
  return {
    buildSpawnSpec,
