...
function createAgentRuntime(deps) {
  ...
  function buildOpencodeSpawnSpec(session, options = {}) {
    ...
    const usePowerShellShim = process.platform === 'win32' && /\.ps1$/i.test(OPENCODE_PATH);
    const command = usePowerShellShim ? 'powershell.exe' : OPENCODE_PATH;
    const args = usePowerShellShim
      ? ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', OPENCODE_PATH, ...cliArgs]
      : cliArgs;
